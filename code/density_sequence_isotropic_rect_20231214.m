...
j = 1;
for i = 1 : size(VE, 1)
    % if VE(i, 1) == 1 || VE(i, 1) == nelx || VE(i, 2) == 1 || VE(i, 2) == nely
    %     continue;
    % end
    [xx, yy] = find(dis(i, :) <= sqrt(2)+1.0e-6);
...
LM = E - L;
LM = sparse(LM);
noncon = unique([1:nely, 1:nely:nely*(nelx-1)+1, nely:nely:nely*nelx, (nelx-1)*nely+1:nely*nelx]);
LM(noncon, :) = [];
...
while loop < nloop 
    ...
    B = sparse(mstart, mstart, ones(1,length(mstart)), size(A, 1), size(A, 2));
    ...
    JJ = JJ(:);
     ...
     %% for density
    S = ((T(edofMatFT)*KET).*repmat(3*k(:).*xPhys(:).^2, 1, 4))';
    dAdx = M*sparse(II, JJ, S(:));

    %% for conductivity
    S = ((T(edofMatFT)*KET).*repmat(xPhys(:).^3, 1, 4))';
    ...
    %%
    % [pdis, dft, dfx] = sizeConstrained(xPhys, tPhys, nelx, nely, nStage, rou, 10);
    ...
    xmin=max(0.0, x(:)-move);
    xmax=min(1, x(:)+move);
    xmin = [xmin; max(k-kmove, 1.0e-6)];
    xmax = [xmax; min(k+kmove, 1.0)];
    xval = [x(:); k(:)];

    %% layer thickness constraint
    A = LM*Tv;
    sk = 1;
    fval = [fval; sk*(sum(A.^2)-1.0e-6)];
    dft = sk*2*(LM)'*A;
    obj = sum(A.^2)-1.0e-6;
    ss = TT*dft;
    alpha = -TA\ss;
    ...
    disp([' It.: ' sprintf('%4i',loop) ' Obj.: ' sprintf('%4.4f',f0val) ...
        ' Vol.: ' sprintf('%4.3f',sum(sum(xPhys))/(tolvol*volfrac))  ' Ch.: ' sprintf('%5.3f', erro)...
